...
public class TaskH {
    ...
    static Node query(int nodeIndex, int lefIndex, int rightIndex, int queryIndex, Node[] tree) {
        int index = tree.length / 2 + queryIndex;

        while (lefIndex < rightIndex) {
            if (tree[nodeIndex].add != 0) {
                ...
                tree[nodeIndex].maxValue = 0;
            }
            int middle = (lefIndex +rightIndex)/2;
            if(queryIndex<=middle) {
...
//        Scanner fs;
//        String fileName = "src/main/resources/lesson2/task8/input.txt";
//        // читаем из input.txt
//        try {
...
//        }
        ...
        for(int i = 0; i < m; i++){
            ...
            }else{ //получить текущее значение элемента по его номеру
                int elemNum = fs.nextInt() - 1;
                Node query = query(0, 0, length - 1, elemNum, tree);
                System.out.println(query.maxValue);
            }
        }
    }
}
