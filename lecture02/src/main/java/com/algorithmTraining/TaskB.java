...
//        String fileName = "src/main/resources/lesson2/input.txt";
//        // читаем из input.txt
//        try {
...
//        }
        ...
        int count = length - n;
        while (count > 0) {
            tree[treeLength - count] = new Node(0, -1);
            count--;
        }
        ...
        for (int i = 0; i < k; i++) {
            ...
            System.out.println(query.index+1);
        }
    }
    ...
    static Node query(int nodeIndex, int lefIndex, int rightIndex, int leftQueryIndex, int rightQueryIndex, Node[] tree) {
        if (leftQueryIndex > rightIndex || rightQueryIndex < lefIndex) return new Node(0, -1);
